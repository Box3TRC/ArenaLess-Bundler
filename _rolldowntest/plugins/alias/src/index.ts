...
function getEntries({ entries, customResolver }:any):any {
  if (!entries) {
    return [];
  }
...
export default function alias(options: any = {}): Plugin {
  ...
  return {
    ...
    resolveId(importee, importer, resolveOptions) {
      ...
      const matchedEntry = entries.find((entry:any) => matches(entry.find, importee));
      if (!matchedEntry) {
        return null;
      }
      ...
      return this.resolve(
        updatedId,
        importer,
        Object.assign({ skipSelf: true }as any, resolveOptions)
      ).then((resolved) => {
